...
if version_extra:
    __version__ = "{0}.{1}.{2}-{3}".format(version_major, version_minor,
                                           version_micro, version_extra)

else:
    __version__ = "{0}.{1}.{2}".format(version_major, version_minor,
                                       version_micro)
...
def get_populse_mia_gitversion():
    ...
    try:
        ...
        dir_mia = os.path.realpath(
            os.path.join(os.path.dirname(populse_mia.__file__),
    ...
    try:
        gitversion, _ = subprocess.Popen("git show -s --format=%h",
                                         shell=True, cwd=dir_mia,

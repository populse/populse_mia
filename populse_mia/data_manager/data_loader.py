...
- Parse and process export logs generated by MRIFileManager.
- Import NIfTI scan files and their associated metadata (e.g., bvec/bval
  files).
- Handle progress tracking and user feedback during the import process.
- Verify the integrity of imported scans via checksum validation.
- Manage database updates for tags, values, and scan metadata.
"""
...
class ImportProgress(QProgressDialog):
    ...
    def __init__(self, project):
        ...
        parent class initialization). Dialog is non-cancelable (no cancel
        button) and appears immediately (minimum duration = 0).
        """
        super().__init__(
...
class ImportWorker(QThread):
    ...
    Contains:
        ...
        Methods:
            ...
            - _apply_default_values: apply default values for user-defined
              tags.
            - _convert_datetime_values: convert string values to datetime
              objects.
            - _ensure_associated_file_tag_exists: ensure the associated file
            ...
            - _process_mrtrix_format_file: process MRtrix format bvec/bval
              file.
            - _process_scan_file: process a single scan file and its
              associated files.

...
class TestMIANodeController(TestMIACase):
    ...
    def test_update_node_name(self):
        ...
        # Try renaming "smooth_1" to an existing name ("smooth_test")
        # — should fail
        simulate_rename("smooth_test")
...
class TestMIAPipelineEditor(TestMIACase):
    ...
    def test_close_tab(self):
        ...
        with patch.object(
            PopUpClosePipeline,
            "exec",
            side_effect=lambda: ppl_edt_tabs.pop_up_close
                .do_not_save_clicked(),
        ):
            # fmt: on
...
class TestMIAPipelineManagerTab(TestMIACase):
        ...
        with patch.object(
            ...
            with (
                ...
                with self.capture_logs(logger_name) as log_handler:
                    ...
                    self.assertTrue(
                        engine_call_found,
                        "'Fake engine.start called' not "
                        "found in info_messages",
                    )
    ...
    def test_undo_redo(self):
        ...
        # Initialize plugs to avoid NoneType
        for node_name in ("smooth_1", "smooth_2"):
            node = pipeline.nodes.get(node_name)

            if node:

                if node.plugs.get("_smoothed_files") is None:
                    node.plugs["_smoothed_files"] = []

                if node.plugs.get("in_files") is None:
                    node.plugs["in_files"] = []
        ...
        # Remove link safely (idempotent), test if the 2 nodes have not
        # the links
        smooth_1 = pipeline.nodes.get("smooth_1")
        smooth_2 = pipeline.nodes.get("smooth_2")

        if smooth_1 and smooth_2:

            if smooth_2.plugs["in_files"].links_from:
                pipeline_editor._del_link(
                    "smooth_1._smoothed_files->smooth_2.in_files"
                )
